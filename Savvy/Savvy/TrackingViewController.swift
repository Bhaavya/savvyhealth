        ...
        let sortedDates = Array(smpLog.keys).sorted{
            ...
            for (idx,date) in sortedDates.enumerated(){
                ...
                if symptomsLogged[sym]![date]![0] == "24"{
                    ...
                    if idx < sortedDates.count - 1 &&
                        dft.string(from: dateFormatterGet.date(from: date)!) == dft.string(from: dateFormatterGet.date(from: sortedDates[idx+1])!)
                    {
                                    ...
                                    durDouble = hours
//                                    print(startDate,endDate,diffComponents,hours,"diff")
                                }
                                else{
                                    durDouble = Double(symptomsLogged[sym]![date]![0])!
                                }
...
//                print(totDur[day],durDouble,Double(symptomsLogged[sym]![date]![0])!,"durb")
                }
    ...
    func drawLineCharts(records: [[String:Any]]){
        ...
        fetchRemoteNote(name: "Note", json: ["chartType":"line","symptomsLoggedLine":symptomsLoggedLine,"dateFormatterNote":dateFormatterNote,"dateFormatterPrint":dateFormatterPrint,"dateFormatterGet":dateFormatterGet] , completion:prepLineChart(notes:json:) )
...
//            print(njson)
            ...
            let dateNote = njson["dateNote"] as! Date
                let dateNoteStr = dateFormatterPrint.string(from: dateNote)
                   let noteSym = njson["symptom"]
          
//            print(dateNoteStr,dateGraphStr)
            if json["chartType"] as! String == "bar" && freq == "w"{
                if dateNote.isInThisWeek(date: dateNote, weekStart: dateGraph) && noteSym as! String == json["symptom"] as! String  {
                    addNote = true
                    anyNote = true
                ...
                if (dateNoteStr == dateGraphStr && noteSym as! String == json["symptom"] as! String )
                {
                    addNote = true
                    anyNote = true
                }
...
//        print(noteTxt)
    ...
    func prepLineChart(notes: [String:[Any]],json:[String:Any]){
        ...
        let dateFormatterPrint = json["dateFormatterPrint"] as! DateFormatter
        let dateFormatterGet = json["dateFormatterGet"] as! DateFormatter
