...
self.searchSym.filterStrings(Array(self.fullList[self.type]!))
   ...
   func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    @IBAction func newSymptomSubmitClicked(_sender: UIButton){
        ...
        if name != nil && name != "" {
            if !self.fullList[self.type]!.contains(name!){
...
//            self.tableView.reloadData()
        self.newSymptomOverlay.removeFromSuperview()
            ...
            let alertController = UIAlertController(title: "Error", message: "Already exists. Enter another description or search this description in the search bar to record it", preferredStyle: .alert)
            ...
            let alertController = UIAlertController(title: "Error", message: "Description must not be empty", preferredStyle: .alert)
...
//            self.saveSymptomLog(name: self.symptomSelected, timestamp:  dateFormatterGet.string(from:datepicker.date), duration: du, durationUnit: dun, intensity: self.intensitySelected,ename: ename)
            ...
            var logging_parameters:[String:AnyObject] = ["id":uid as AnyObject,"page":"logging" as AnyObject,"action":"log" as AnyObject,"json":["name":self.symptomSelected, "timestamp":dateFormatterGet.string(from:datepicker.date),
                 "duration":du,"durationUnit": dun, "intensity": Int(self.intensitySelected),"ename": ename] as AnyObject]
...
//    self.saveSymptomLog(name: self.symptomSelected, timestamp:  dateFormatterGet.string(from:datepickerMood.date), duration: "24", durationUnit: "hrs", intensity: Float(score),ename: ename)
            ...
            var logging_parameters:[String:AnyObject] = ["id":uid as AnyObject,"page":"logging" as AnyObject,"action":"log" as AnyObject,"json":["name":self.symptomSelected, "timestamp":dateFormatterGet.string(from:datepickerMood.date),
                 "duration":"24","durationUnit": "hrs", "intensity": Int(score),"ename": ename] as AnyObject]
